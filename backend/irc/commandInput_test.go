...
func TestArgTypeChannelOrNick(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		valid    bool
		...
		{
			name:     "Valid channel starting with number",
			input:    "#123test",
			valid:    true,
			expected: "#123test",
		},
			...
			args := []Argument{
				{
					...
					Required:    true,
					Description: "Target channel or nickname",
				},
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.windowID != "" {
				window = &Window{
					id:        tt.windowID,
					isChannel: tt.windowIsChannel,
				}
			}

			parsed, err := Parse(cmd, tt.input)
			assert.NoError(t, err)

			result, err := parsed.GetArgStringWithChannelFallback("channel", window)
...
func TestGetArgStringWithTargetFallback(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := parsed.GetArgStringWithTargetFallback("target", window)
